...
import { createRole, getRoleDetail, updateRole, updateRoleNoUpdatePermission } from '../../../service/RoleService';
import { toast, ToastContainer } from 'react-toastify';

const EditGroupComponent = ({ id }) => {
    const [selectedValue, setSelectedValue] = useState({}); // permission chon
    const [selectedRole, setSelectedRole] = useState({});  // admin va default
    const [isChecked, setIsChecked] = useState({}); // check o cac permission chon
    const [permissionError, setPermissionError] = useState({});
    const [prevPermission, setPrevPermission] = useState({});
    const [role, setRole] = useState(); //role detail
    ...
    useEffect(() => {
        ...
        if (role) {
            ...
            setPrevPermission(role.permissions)
            if (role.accountAdmin) {
                setSelectedRole("admin")
            } else if (role.accountDefault) {
                setSelectedRole("default")
            } else {
                const permissionsList = role.permissions.split(",").map(num => parseInt(num.trim()));
    ...
    const handleSubmitUpdateGroup = () => {
        ...
        const updateFunction = isSamePermissions ? updateRoleNoUpdatePermission : updateRole;
        updateFunction(id, values.name, values.code, values.desc, listPermission === "" ? selectedRole : listPermission)
            .then((response) => {
                if (response.data.code === 1000) {
                    ...
                    <div className="modal-content">
                        <div className="modal-header">
                            <div className="d-flex align-items-center">
                                <h4 className="modal-title me-2">Sửa nhóm người dùng + {id}</h4>
                            </div>
                            ...
                            <div className="tab-content" id="myTabContent">
                                <div className="tab-pane fade show active">
                                        ...
                                        <div className="row">
                                            <div className="col-md-8">
                                                <div className="mb-3">
                                                    ...
                                                    <input type="text" className="form-control" name='desc' onChange={onChangeInput} value={values.desc || ""} />
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="modal-footer">
                                        ...
                                        <div className="btn btn-primary" onClick={handleSubmitUpdateGroup}>CẬP NHẬT  </div>
                                    </div>
                                </div>
                            </div>
                        </form>
